...
--   • variants: jsonb array of { name, price_delta } — the line's unit_price
--     ALREADY includes the chosen deltas; the array is kept for the ticket.
--   • vat_rate: % IVA of the item, receipts print the scorporo per rate.
--     10 = somministrazione (default for restaurants).
--   • station: which printer/prep area the line belongs to (cucina, bar…).
