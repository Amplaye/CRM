...
> **⛳ FASE DI COSTRUZIONE CHIUSA (2026-05-21, decisione utente "è pronto, ora servono clienti").** Tutto il costruibile pre-cliente è fatto e verde (`tsc` 0, 97/97 test). Le uniche due cose rimaste sono **gated sul primo cliente vero** (non debito tecnico, scelta esplicita): **Mossa 5 parte Twilio** (comprare/collegare un numero WhatsApp reale + routing n8n in ingresso — codice già pronto, è una riga di config) e **Mossa 6** (motore unico, da fare solo quando il registro varianti si stabilizza, ~5-15 clienti). **Da qui il progresso vero è commerciale, non altro codice.** Non riaprire la fase di costruzione inventando varianti che nessun cliente ha chiesto (= sintomo-agenzia che il piano vuole evitare). Vedi la riga in cima al Log per i dettagli dell'indagine già chiusa (es. perché `double_shift`/`multi_room` NON vanno nel motore).
...
>
> **Aggiornamento 2026-05-21 (sera) — il MOTORE ora rispetta `waitlist_enabled`, non solo la UI.** Scoperta: finora spegnere "Lista d'attesa" nascondeva *solo* la voce nella Sidebar del CRM — il motore AI continuava a mettere ospiti in lista e a offrire tavoli. Il criterio di verifica della Mossa 3 ("flip flag → comportamento cambia") era quindi soddisfatto solo a metà. **Chiusura del gap, lato motore che controlliamo oggi (le route Next.js `/api/ai/*`, NON l'n8n/Retell che resta Mossa 6):** nuovo helper unico server-side `src/lib/tenants/features.ts` (`getTenantFeatures`, fail-open ai default su tenant mancante). Gate aggiunti: `/api/ai/waitlist` (flag off → 409 `waitlist_disabled`, niente coda), `/api/ai/waitlist-process` (flag off → `matched:0`, niente offerte), e i **3 fallback "pieno → lista d'attesa" di `/api/ai/book`** (flag off → declina onestamente con `status:'full'` e propone altra data/ora, invece di accodare in silenzio). Default ON → comportamento attuale invariato. **Test loop live** (tenant usa-e-getta, poi cancellato): flag OFF → add 409 + process matched:0 + 0 voci create; flag ON → add 200 + voce creata. tsc 0, 80/80 test (+4 su `getTenantFeatures`), `next build` 0. **Nota onestà:** `/api/ai/waitlist-reassurance` (ping cortesia a chi è già in coda) NON è gated — agisce solo su voci *esistenti*, e con la porta d'ingresso ora chiusa non se ne creano di nuove a flag off; edge case (disattivare con voci pendenti) lasciato all'owner.
...
> **✅ FATTO — Numero di invio "pronto per il cliente" (2026-05-21 sera).** Prima il numero WhatsApp da cui si **invia** era un solo valore globale (env `TWILIO_WHATSAPP_FROM`, fallback sandbox) copiato a mano in **4 file** → "stesso numero per tutti", il sintomo-agenzia. Ora il numero di invio è **config per-tenant** (`settings.whatsapp.from`), risolto in **un unico punto** (`src/lib/whatsapp/from.ts` → `resolveWhatsAppFrom`, ordine: numero del tenant → env piattaforma → sandbox). Collegato a tutte le vie d'invio: `/api/send-whatsapp` (le 4 pagine dashboard ora passano `tenant_id`), il motore lista d'attesa (`waitlist-process` cliente+owner, `waitlist-reassurance` per-tenant in batch). `admin/bali/send` lasciato com'è (numero proprio dell'agenzia, prodotto separato). **Oggi comportamento identico** (nessun tenant ha ancora un numero proprio → tutti risolvono allo stesso sandbox/env). Quando arriva il primo cliente con numero vero, **è una riga di config, non una modifica al codice** — questo È il trampolino. Bloccato dal build: nessuna route può più scrivere a mano il numero sandbox (`saas-invariants.test.ts`). tsc 0, 97/97 test (+8). Lato ricezione già per-tenant (n8n posta `tenant_id` a `incoming-message`).

> **⏸️ RESTA — Acquisto/assegnazione del numero reale (gated ai clienti reali).** Oggi un solo numero attivo (sandbox WhatsApp = 1 webhook, limite fisico) — vedi `reference_onboard_wizard.md`. Il **codice è pronto** (sopra): manca solo (a) comprare/collegare un numero WhatsApp dedicato e scriverlo in `settings.whatsapp.from`, (b) la config di routing in ingresso su n8n per quel numero. Niente codice nuovo "al buio" — si fa quando arriva il primo cliente vero.
...
- 2026-05-21 (notte) — **Tetto pre-cliente raggiunto: fase di costruzione chiusa** (decisione utente: "è pronto, ora servono clienti"). Nessun codice toccato questa sessione — solo verifica + decisione. **Verificato verde:** `tsc` 0, 97/97 test. **Indagine chiusa (così non si rifà):** ho controllato se andava collegato al motore anche `double_shift` e `multi_room`, come fatto con `waitlist_enabled`. **NON serve, di proposito:** il motore (`/api/ai/availability`, `/api/ai/book`) decide turni e zone leggendo `settings.opening_hours` e la colonna `tables.zone` dal DB del singolo tenant (`getShift`/`isOpen`/`classifyHora` in `restaurant-rules.ts` sono interamente data-driven). Quei due flag sono concetti di **presentazione CRM** (selettore Pranzo/Cena, pulsante aggiungi-zona), non gate del motore — aggiungere un gate lì introdurrebbe complessità invece di togliere un sintomo-agenzia. Le route `/api/ai/*` sono **già un motore unico condiviso** tenant-parametrizzato (non clonato) → la parte di motore che controlliamo in-process è già SaaS-corretta. L'onboarding (`orchestrator.ts`) è **interamente automatico** (tenant→tavoli→agente Retell→sync KB→clona+attiva 13 workflow→account owner), nessuno step manuale nel codice. **Cosa resta, tutto gated sul primo cliente vero (decisione utente esplicita, non debito tecnico):** (5) comprare/collegare un numero WhatsApp reale + routing n8n in ingresso — codice già pronto, è una riga di config; (6) motore unico al posto dei 13 workflow clonati — si fa solo quando il registro varianti si stabilizza (5-15 clienti). Da qui il progresso vero è **commerciale**, non altro codice. — nessun commit di codice (solo questa riga di doc)
- 2026-05-21 (sera) — Mossa 5 (parte Twilio: codice "pronto per il numero del cliente") — chiuso il pezzo NON bloccato di Mossa 5 senza comprare numeri. Il numero WhatsApp da cui si **invia** era globale (env + fallback sandbox) copiato in **4 file** → "un numero per tutti" (sintomo-agenzia). Ora è config per-tenant `settings.whatsapp.from`, risolto in un unico punto `src/lib/whatsapp/from.ts` (`resolveWhatsAppFrom`: tenant → env → sandbox; helper `tenantWhatsAppFrom` legge i settings in sicurezza). Collegato ovunque: `/api/send-whatsapp` (+ `tenant_id` dalle 4 pagine dashboard: reservations/waitlist/conversations/pending), `waitlist-process` (cliente+owner, numero letto dai settings già caricati, zero query extra), `waitlist-reassurance` (per-tenant in batch, una sola query per lo sweep). `admin/bali/send` non toccato (numero proprio dell'agenzia Bali Flow, prodotto separato). **Comportamento oggi identico** (nessun tenant ha un numero proprio → tutti allo stesso sandbox/env); col primo cliente vero diventa una riga di config, non codice = il trampolino. Nuova invariante in `saas-invariants.test.ts`: il numero sandbox non può ricomparire in nessuna route `src/app/api/**`. **Verifica:** tsc 0, 97/97 test (+8: 7 su `resolveWhatsAppFrom`/`tenantWhatsAppFrom`, 1 invariante anti-regressione), build 0. Nessun test live d'invio (comportamento invariato per i tenant esistenti; un invio reale con numero finto genererebbe solo errori Twilio). Mossa 5 resta 🔄: manca solo comprare/assegnare un numero reale + routing n8n in ingresso, gated ai clienti. — commit 2d74bb3
- 2026-05-21 (sera) — Verifica Globale → automatizzata (hardening, nessuna mossa nuova) — la checklist manuale "VERIFICA GLOBALE" è ora in parte un **controllo automatico** (`npm test`): i pilastri provabili in-process diventano test che bloccano il build se qualcuno li rompe. Nuovi: `src/lib/tenants/create-tenant.test.ts` (vertical unico `business_type=restaurant` + status esplicito alla nascita) e `src/lib/saas-invariants.test.ts` (scan anti-regressione: il numero del template `34641790137` non deve comparire in nessuna route `src/app/api/**`, i costanti-fallback rimossi in Mossa 1 restano rimossi, e un flag cambia solo il tenant che lo flippa). Doc investitore `docs/PROVA_SAAS.md` (cosa è garantito dal build vs cosa resta manuale/gated ai servizi esterni). **Zero codice di produzione toccato** → solo nuovi test + doc, app identica. tsc 0, 89/89 test (+9), build non necessario (nessun file app cambiato). I pilastri che toccano servizi esterni (Retell/n8n/Twilio) restano verifiche manuali in attesa del primo cliente reale. — commit 1b1f6b0
- 2026-05-21 (sera) — Mossa 3 (estensione: motore rispetta i flag) — chiuso il gap "flag cambia solo la UI": il **motore** (route Next.js `/api/ai/*`) ora rispetta `waitlist_enabled`. Nuovo helper unico `src/lib/tenants/features.ts` (`getTenantFeatures`, fail-open). Gate in `/api/ai/waitlist` (flag off → 409 `waitlist_disabled`), `/api/ai/waitlist-process` (off → `matched:0`), e i 3 fallback "pieno→lista" di `/api/ai/book` (off → `status:'full'`, propone altra data/ora, niente coda silenziosa). Default ON = comportamento invariato. n8n/Retell resta Mossa 6. `waitlist-reassurance` non gated (agisce solo su voci esistenti). Test loop live (tenant usa-e-getta poi cancellato): off→add 409 + process 0 + 0 voci; on→add 200 + voce creata. tsc 0, 80/80 (+4 su `getTenantFeatures`), build 0. — commit 0a77cda
- 2026-05-21 — Mossa 3 (estensione: interruttori → comportamento) — collegati i flag finora "decorativi" a comportamenti REALI, così ogni interruttore visibile fa qualcosa. **`multi_room`** → pulsante "aggiungi zona/sala" nella pianta tavoli (`floor/page.tsx`): se off = ristorante a sala unica, niente gestione zone; se on = puoi crearle. Sicuro: le zone già esistenti restano sempre visibili (`canManageZones = features.multi_room || zones.length > 1`), niente sparisce. **`double_shift`** → selettore Pranzo/Cena (`floor/page.tsx`): se off = servizio unico, il toggle sparisce e si vedono tutte le prenotazioni del giorno insieme (mai nascoste). Default on → comportamento invariato. **`terrace`/`pet_friendly`/`events_enabled`/`multi_language`** → sono "fatti" che l'assistente racconta agli ospiti: nuovo helper `restaurantFacts()` in `tenant-settings.ts` e blocco `facts` aggiunto a ogni risposta di `/api/ai/restaurant-info` (config → l'assistente li legge, non testo KB scritto a mano per cliente). **Decisione utente (cameriere esperto):** NON rimuovere i 4 flag-fatto (alta probabilità di richiesta dai ristoranti reali → front-load capacità ora = meno lavoro dopo); diverso dalle "porte dipinte" della Mossa 1B (quelle erano verticali sbagliati, mai in arrivo; questi sono feature giuste in arrivo). Verifica: tsc 0, 76/76 test (+2 su `restaurantFacts`), `next build` 0, curl live a `restaurant-info` → blocco `facts` presente e derivato dai flag del tenant (nessun dato mutato). — commit 1afb950
