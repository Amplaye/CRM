...
**One deferred follow-up (not a new risk):** strip the secret keys from `settings.bot_config` (already mirrored into the protected `tenants.secrets`) once the n8n read-path is pointed at `tenants.secrets` — deferred to avoid interrupting the live Picnic bot. Until then secrets remain readable by a tenant member via `settings`; customer PII/data is fully protected.
