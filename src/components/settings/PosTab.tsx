  ...
  return (
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
          {POS_OPTIONS.map((opt) => (
            <button
              ...
              disabled={!opt.live}
              className={`px-3 py-2.5 text-sm rounded-lg border-2 text-left transition-colors ${choice === opt.id ? "font-bold text-black" : "text-black/70"} ${opt.live ? "cursor-pointer hover:bg-[#c4956a]/10" : "opacity-50 cursor-not-allowed"}`}
              style={{ borderColor: choice === opt.id ? "#c4956a" : "#eaddcb", background: choice === opt.id ? "rgba(196,149,106,0.12)" : undefined }}
            >
              {opt.label}
              {!opt.live && <span className="block text-[10px] text-black/40">{t("settings_pos_soon" as keyof Dictionary) || "presto"}</span>}
            </button>
          ))}
        </div>
      </div>
      ...
      {isLive ? (
        ...
        <div className="rounded-lg border-2 p-4 text-sm text-black/70" style={{ borderColor: "#eaddcb", background: "rgba(252,246,237,0.4)" }}>
