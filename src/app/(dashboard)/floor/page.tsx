...
// Floor patterns: a key + a CSS background. Most are seamless image tiles under
// public/floors/ (generated by scripts/gen-floor-textures.mjs — owned, brand-
// matched, no external dependency). `grid` true keeps the faint dot grid for the
// plain option; image patterns hide it. `size` sets the tile display size.
type FloorTexture = { key: string; labelKey: string; css: string; size?: string; grid: boolean };

const FLOOR_TEXTURES: FloorTexture[] = [
  {
    // Plain warm cream — a blank floor (keeps the placement dot-grid).
    key: "neutral",
    labelKey: "floor_tex_neutral",
    css: "linear-gradient(160deg, #fbf4e9, #efe2cd)",
    size: "auto",
    grid: true,
  },
  {
...
function PlanCanvas({ tables, resTableLinks, shiftReservations, activeStatuses, editing, wallMode, walls, floorTexture, onAddWall, onUpdateWall, onDeleteWall, onTableMove, onTableRotate, onTableClick, t }: PlanCanvasProps) {
  ...
  function endWallDrag() {
    ...
    setWallDrag(null);
    // Swallow the click that the browser fires right after this mouseup, so
    // releasing a wall (in wall mode) doesn't immediately start drawing a new one.
  ...
  return (
    <div
      ...
      onClick={(e) => {
        ...
        // Ignore the click synthesized right after a wall move/reshape release.
        if (wallJustDraggedRef.current) { wallJustDraggedRef.current = false; return; }
    ...
    >
      {/* Background plane fills the canvas (texture/pattern lives here). */}
      <div
