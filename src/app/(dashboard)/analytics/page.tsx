...
export default function AnalyticsPage() {
  ...
  const kpis = useMemo(() => {
    ...
    // Web bookings (booking widget on the public site, source='web') are a
    // channel of their own — neither AI-chat/voice nor manual staff. They're
    // generated by the system, so their covers count toward gross value, but
    // they're reported as "Web" in the channel split so the user can see the
    // widget is producing bookings.
    const webRes = reservations.filter((r) => r.source === "web");
    const webResPaid = webRes.filter((r) => r.status !== "no_show" && r.status !== "cancelled");
    const webRevenue = webResPaid.reduce((sum, r) => sum + (r.party_size || 0) * avgSpend, 0);
    ...
    // Net AI Value = gross − cost (floor at 0). Web-widget covers are part of
    // the gross the system generated (the widget is TableFlow, not staff).
    const grossValue = Math.round(aiRevenue + webRevenue + waitlistMatchRevenue + noShowSaved);
    ...
    // "Handled by the system" = AI channels + web widget (everything not manual).
    const systemHandled = aiRes.length + webRes.length;
    const aiHandledPct = total > 0 ? Math.round((systemHandled / total) * 100) : 0;
    ...
    return {
      ...
      webCount: webRes.length,
      staffCount: total - aiRes.length - webRes.length,
      totalConversations,
      totalIncidents,
      // Charts
      chartData,
  ...
  const reportData = () => {
    const summary: Array<{ label: string; raw: string | number; pretty: string }> = [
      ...
      { label: t("analytics_ai_handled"), raw: kpis.aiHandledPct, pretty: `${kpis.aiHandledPct}% (${kpis.aiCount} AI / ${kpis.webCount} ${t("analytics_web_handled")} / ${kpis.staffCount} Staff)` },
      ...
      >
        ...
        >
          {[
            ...
            { icon: Gauge, label: t("analytics_ai_handled"), value: `${kpis.aiHandledPct}%`, sub: `${kpis.aiCount} AI / ${kpis.webCount} ${t("analytics_web_handled")} / ${kpis.staffCount} Staff` },
