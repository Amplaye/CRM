      ...
      <div className={`flex-1 transition-all duration-300 ${isCreating ? 'pr-[400px]' : ''}`}>
          ...
          <div className="mb-8 relative overflow-hidden rounded-2xl border-2 flex items-start sm:items-center p-6 justify-between flex-col sm:flex-row gap-4" style={{ background: 'rgba(252,246,237,0.85)', borderColor: '#c4956a', boxShadow: '0 20px 60px rgba(196,149,106,0.25), 0 8px 24px rgba(196,149,106,0.15)' }}>
             ...
             <div className="flex items-start relative z-10 w-full">
                ...
                <div className="ml-5 flex-1">
                   <h3 className="text-lg font-bold text-zinc-900 tracking-tight">{t("waitlist_recovery_match")}</h3>
                   <p className="text-sm text-zinc-600 mt-1 max-w-2xl leading-relaxed">
                     {t("waitlist_cancellation_match")} <span className="font-bold text-zinc-900">{matchFoundEntry.guests?.name || `Guest ${matchFoundEntry.guest_id.substring(0,8)}`}</span> ({matchFoundEntry.party_size} pax). Their flexible window is {matchFoundEntry.acceptable_time_range.start} – {matchFoundEntry.acceptable_time_range.end}.
                   </p>
        ...
        <div className="border-2 rounded-xl overflow-hidden" style={{ background: 'rgba(252,246,237,0.85)', borderColor: '#c4956a', boxShadow: '0 20px 60px rgba(196,149,106,0.25), 0 8px 24px rgba(196,149,106,0.15)' }}>
           <div className="px-6 py-4 border-b flex justify-between items-center" style={{ borderColor: '#c4956a' }}>
              ...
              <span className="bg-zinc-200 text-zinc-800 text-[10px] uppercase tracking-wider font-bold px-2.5 py-1 rounded-md">{entries.length} {t("waitlist_checking")}</span>
           </div>

           {entries.length === 0 && !loading ? (
              <div className="py-16 text-center text-black">
                 <Activity className="w-12 h-12 text-zinc-300 mx-auto mb-4" />
                 <p className="text-sm font-bold text-zinc-900">{t("waitlist_no_entries")}</p>
                 <p className="text-xs text-zinc-500 mt-1">{t("waitlist_no_management")}</p>
              </div>
           ) : loading ? (
             ...
             <table className="min-w-full divide-y divide-zinc-200">
               <thead className="bg-white">
                 <tr>
                   ...
                   <th scope="col" className="px-3 sm:px-6 py-3 text-left text-[10px] font-bold text-zinc-500 uppercase tracking-widest">{t("waitlist_target_flex")}</th>
                   <th scope="col" className="px-3 sm:px-6 py-3 text-left text-[10px] font-bold text-zinc-500 uppercase tracking-widest">{t("waitlist_status_score")}</th>
      ...
      {isCreating && (
        <div className="fixed inset-y-0 right-0 w-full sm:w-[400px] border-l shadow-2xl z-40 transform transition-transform duration-300 flex flex-col" style={{ background: 'rgba(252,246,237,0.95)', borderColor: '#c4956a' }}>
          <div className="px-6 py-4 flex items-center justify-between border-b" style={{ borderColor: '#c4956a' }}>
             <h2 className="text-lg font-bold text-zinc-900 tracking-tight">{t("waitlist_registration")}</h2>
          ...
          <form onSubmit={handleCreate} className="flex-1 flex flex-col overflow-hidden">
             <div className="flex-1 overflow-y-auto p-6 space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                   <div>
                     <label className="block text-xs font-bold text-zinc-700 mb-1">{t("waitlist_guest_name")}</label>
                   ...
                   <div>
                     <label className="block text-xs font-bold text-zinc-700 mb-1">{t("waitlist_guest_phone")}</label>
                ...
                <div className="grid grid-cols-2 gap-4">
                   <div>
                     <label className="block text-xs font-bold text-zinc-700 mb-1">{t("waitlist_date")}</label>
                   ...
                   <div>
                     <label className="block text-xs font-bold text-zinc-700 mb-1">{t("waitlist_party_size")}</label>
                ...
                <div className="border border-zinc-200 rounded-lg p-4 bg-white shadow-sm">
                   <h3 className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest mb-3">{t("waitlist_time_flex")}</h3>
                ...
                <div>
                   <label className="block text-xs font-bold text-zinc-700 mb-1">{t("waitlist_contact_pref")}</label>
                ...
                <div>
                   <label className="block text-xs font-bold text-zinc-700 mb-1">{t("waitlist_internal_notes")}</label>
                   <textarea
                     name="notes"
                     rows={2}
