...
async function resolveCard(row: SystemLogRow) {
  ...
  // Move to the done list (Hecho) and leave a trace.
...
// Sweep: archive cards in the done list whose last activity is older than N days.
 ...
 * - new open row  → create card in the active-bugs list
 * - row -> resolved -> move card to the done list
...
// When a human drags a card into the done list ON TRELLO, close the matching
...
// The done ("Hecho") list id — exported so the inbound webhook can tell whether a
