...
export const KB: KbTopic[] = [
  ...
  },
  {
    ...
    answer: {
      it: "Nel Food Cost colleghi a ogni piatto la sua ricetta (ingredienti e quantità): il CRM calcola costo, margine e incidenza, e ti suggerisce il prezzo giusto. In alto vedi il profitto stimato a 30 giorni (margine × venduti), i soldi che stai lasciando sul tavolo (quanto recupereresti al mese alzando i piatti sotto-prezzo) e il food cost medio pesato sulle vendite reali. Puoi ordinare i piatti per «Prima da sistemare», «Più redditizi», «Food cost % peggiore» o «Più venduti»; la stella ⭐ segna i cavalli di battaglia che reggono il profitto. I costi ingredienti arrivano dall'Inventario e si aggiornano da soli.",
      en: "In Food Cost you link each dish to its recipe (ingredients and quantities): the CRM computes cost, margin and incidence, and suggests the right price. At the top you see estimated 30-day profit (margin × units sold), the money you're leaving on the table (what you'd recover per month by repricing under-priced dishes) and the average food cost weighted by real sales. You can sort dishes by 'To do first', 'Most profitable', 'Worst food cost %' or 'Best sellers'; the ⭐ marks the menu stars that carry the profit. Ingredient costs come from Inventory and update automatically.",
      es: "En Food Cost vinculas cada plato a su receta (ingredientes y cantidades): el CRM calcula coste, margen e incidencia, y te sugiere el precio adecuado. Arriba ves el beneficio estimado a 30 días (margen × vendidos), el dinero que dejas sobre la mesa (lo que recuperarías al mes ajustando los platos mal preciados) y el food cost medio ponderado por ventas reales. Puedes ordenar los platos por 'Primero por resolver', 'Más rentables', 'Peor food cost %' o 'Más vendidos'; la ⭐ marca los platos estrella que sostienen el beneficio. Los costes vienen del Inventario y se actualizan solos.",
      de: "Im Food Cost verknüpfst du jedes Gericht mit seinem Rezept (Zutaten und Mengen): das CRM berechnet Kosten, Marge und Anteil und schlägt den richtigen Preis vor. Oben siehst du den geschätzten 30-Tage-Gewinn (Marge × verkaufte Einheiten), das Geld, das du auf dem Tisch liegen lässt (was du monatlich zurückholst, indem du zu günstige Gerichte anpasst) und den nach echten Verkäufen gewichteten Wareneinsatz. Du kannst Gerichte nach 'Zuerst erledigen', 'Rentabelste', 'Schlechtester Wareneinsatz %' oder 'Bestseller' sortieren; der ⭐ markiert die Menü-Stars, die den Gewinn tragen. Zutatenkosten kommen aus dem Inventar und aktualisieren sich automatisch.",
    },
