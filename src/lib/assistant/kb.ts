...
export const KB: KbTopic[] = [
  ...
  },
  {
    ...
    keywords: [
      ...
      "rinominare tavolo", "cambiare nome tavolo", "nome del tavolo", "rename table", "table name", "renombrar mesa", "cambiar nombre mesa", "nombre de la mesa", "tisch umbenennen", "tischname",
    ],
    answer: {
      it: "Nella pagina Sala disegni la mappa del locale: aggiungi tavoli, imposti posti e forma, li raggruppi in zone (es. dehors, sala interna). Puoi anche rinominare un tavolo quando vuoi (es. da T3 a \"Veranda 1\"): il nome cambia ovunque, senza perdere prenotazioni o storico. Gli stessi tavoli vengono usati dalle prenotazioni e dalla cassa.",
      en: "On the Floor page you draw your room map: add tables, set seats and shape, group them into zones (e.g. terrace, main room). You can also rename a table any time (e.g. from T3 to \"Terrace 1\"): the name changes everywhere, with no loss of bookings or history. The same tables are used by reservations and by the till.",
      es: "En la página Sala dibujas el plano del local: añades mesas, defines plazas y forma, y las agrupas en zonas (ej. terraza, sala interior). También puedes renombrar una mesa cuando quieras (ej. de T3 a \"Terraza 1\"): el nombre cambia en todas partes, sin perder reservas ni histórico. Las mismas mesas se usan en reservas y en la caja.",
      de: "Auf der Seite Saal zeichnest du den Raumplan: Tische hinzufügen, Plätze und Form festlegen, in Zonen gruppieren (z. B. Terrasse, Innenraum). Du kannst einen Tisch jederzeit umbenennen (z. B. von T3 zu \"Terrasse 1\"): der Name ändert sich überall, ohne Reservierungen oder Historie zu verlieren. Dieselben Tische nutzen Reservierungen und Kasse.",
    },
    steps: {
      it: ["Apri Sala dal menu laterale", "Aggiungi un tavolo e trascinalo dove vuoi sulla mappa", "Imposta nome, numero di posti e zona", "Per rinominarlo: nella vista elenco tocca la matita accanto al nome, oppure in Modifica mappa tocca il tavolo e usa il campo del nome", "Salva: il tavolo appare subito anche in Cassa e Prenotazioni"],
      en: ["Open Floor from the sidebar", "Add a table and drag it anywhere on the map", "Set name, seats and zone", "To rename it: in list view tap the pencil next to the name, or in Edit map tap the table and use the name field", "Save: the table immediately appears in the Till and Reservations too"],
      es: ["Abre Sala en el menú lateral", "Añade una mesa y arrástrala en el plano", "Define nombre, plazas y zona", "Para renombrarla: en la vista de lista toca el lápiz junto al nombre, o en Editar plano toca la mesa y usa el campo del nombre", "Guarda: la mesa aparece al momento también en Caja y Reservas"],
      de: ["Öffne Saal in der Seitenleiste", "Füge einen Tisch hinzu und ziehe ihn auf dem Plan", "Lege Name, Plätze und Zone fest", "Zum Umbenennen: in der Listenansicht auf den Stift neben dem Namen tippen, oder im Plan-Bearbeitungsmodus den Tisch antippen und das Namensfeld nutzen", "Speichern: der Tisch erscheint sofort auch in Kasse und Reservierungen"],
    },
  ...
  },
  {
    ...
    keywords: [
      "menu digitale", "qr", "qr code", "codice qr", "link menu", "menu online", "digital menu", "menu publico", "menu link", "carta digital", "online karte",
      "filtri menu", "filtrare piatti", "tag piatti", "vegano", "senza glutine", "menu filters", "filter dishes", "dish tags", "filtros carta", "filtrar platos", "etiquetas platos", "karte filter", "gerichte filtern",
    ],
    answer: {
      it: "Il tuo menu ha una pagina pubblica con link e QR code da mettere sui tavoli: i clienti la aprono dal telefono, senza app. Si aggiorna da sola ogni volta che modifichi il menu nel CRM. I clienti possono anche filtrare i piatti toccando i tag (vegano, piccante, consigliato…): compaiono da soli solo se hai taggato almeno un piatto.",
      en: "Your menu has a public page with a link and QR code to put on tables: guests open it on their phone, no app needed. It updates automatically whenever you edit the menu in the CRM. Guests can also filter dishes by tapping the tags (vegan, spicy, recommended…): the filters appear on their own only if you tagged at least one dish.",
      es: "Tu menú tiene una página pública con enlace y código QR para poner en las mesas: los clientes la abren desde el móvil, sin app. Se actualiza sola cada vez que editas el menú en el CRM. Los clientes también pueden filtrar los platos tocando las etiquetas (vegano, picante, recomendado…): los filtros aparecen solos únicamente si has etiquetado algún plato.",
      de: "Deine Speisekarte hat eine öffentliche Seite mit Link und QR-Code für die Tische: Gäste öffnen sie am Handy, ohne App. Sie aktualisiert sich automatisch, sobald du die Karte im CRM änderst. Gäste können Gerichte auch über die Tags filtern (vegan, scharf, empfohlen…): die Filter erscheinen nur, wenn du mindestens ein Gericht getaggt hast.",
    },
  ...
  },
  {
    ...
    keywords: [
      "inventario", "magazzino", "scorte", "ingredienti", "carico", "fattura foto", "stock", "inventory", "ingredients", "stock-in", "supplier order", "ordine fornitore", "fornitore", "fornitori", "existencias", "almacen", "proveedor", "lager", "bestand", "lieferant", "sprechi", "shrinkage", "scorta minima", "par level", "sottoscorta", "categoria", "categorie", "categorie magazzino", "carne", "verdura", "latticini", "bevande", "unità di misura", "unita di misura", "grammi", "litri", "confezione", "cartone", "category", "categories", "unit of measure", "units", "pack", "case", "categoría", "categorías", "unidad de medida", "unidades", "kategorie", "kategorien", "maßeinheit", "einheiten", "magazzino predefinito", "ingredienti predefiniti", "carica magazzino", "magazzino vuoto", "ingredienti di base", "default storeroom", "default ingredients", "load storeroom", "empty inventory", "starter ingredients", "almacén predeterminado", "almacen predeterminado", "ingredientes predeterminados", "cargar almacén", "inventario vacío", "standardlager", "standardzutaten", "lager laden", "leeres inventar",
    ],
    answer: {
      it: "L'Inventario tiene le scorte degli ingredienti quasi da solo: fotografi la fattura del fornitore e il carico viene registrato automaticamente (creando anche gli ingredienti nuovi); le scorte minime si calcolano da sole e quando un ingrediente va sottoscorta puoi generare l'ordine al fornitore da inviare su WhatsApp. C'è anche il pannello sprechi per capire dove si perde merce. Gli ingredienti sono ordinati per categoria di magazzino (Carne, Pesce, Verdura, Latticini, Vino, Materiale di consumo…): la categoria viene assegnata da sola dal nome del prodotto e puoi correggerla aprendo la riga. I pulsanti categoria in alto filtrano la lista, che mostra 20 prodotti per pagina. Ogni prodotto ha la sua unità di misura scelta da un elenco completo (mg, g, hg, kg, ml, cl, l, cucchiaio, pezzi, confezioni, cartoni, bottiglie…): se cambi unità, scorte, scorta minima e costo vengono convertiti automaticamente. Se il magazzino è vuoto, il pulsante «Carica il magazzino predefinito» lo riempie con gli ingredienti base di una cucina professionale, già tradotti nella lingua del CRM e già categorizzati; i costi restano a zero finché non li inserisci tu o non arrivano dalle fatture. I ristoranti nuovi lo ricevono già pronto alla creazione.",
      en: "Inventory keeps ingredient stock almost by itself: photograph the supplier invoice and the stock-in is recorded automatically (new ingredients are created too); minimum levels are computed automatically and when an ingredient runs low you can generate the supplier order to send on WhatsApp. A shrinkage panel shows where goods get lost. Ingredients are filed by warehouse category (Meat, Fish, Vegetables, Dairy, Wine, Consumables…): the category is assigned automatically from the product name and you can correct it by opening the row. The category buttons at the top filter the list, which shows 20 products per page. Each product has its unit picked from a full catalogue (mg, g, hg, kg, ml, cl, l, tablespoon, pieces, packs, cases, bottles…): change the unit and stock, minimum level and cost are converted automatically. If the warehouse is empty, the “Load the default storeroom” button fills it with the staple ingredients of a professional kitchen, already translated into the CRM language and already categorised; costs stay at zero until you enter them or they arrive from invoices. New restaurants get it ready-made at creation.",
      es: "El Inventario lleva las existencias casi solo: fotografías la factura del proveedor y la carga se registra automáticamente (creando también los ingredientes nuevos); los mínimos se calculan solos y cuando un ingrediente baja de stock puedes generar el pedido al proveedor para enviarlo por WhatsApp. El panel de mermas muestra dónde se pierde género. Los ingredientes se ordenan por categoría de almacén (Carne, Pescado, Verduras, Lácteos, Vino, Consumibles…): la categoría se asigna sola a partir del nombre y puedes corregirla abriendo la fila. Los botones de categoría de arriba filtran la lista, que muestra 20 productos por página. Cada producto tiene su unidad elegida de un catálogo completo (mg, g, hg, kg, ml, cl, l, cucharada, unidades, paquetes, cajas, botellas…): si cambias la unidad, existencias, mínimo y coste se convierten automáticamente. Si el almacén está vacío, el botón «Cargar el almacén predeterminado» lo llena con los ingredientes básicos de una cocina profesional, ya traducidos al idioma del CRM y ya categorizados; los costes quedan a cero hasta que los introduzcas o lleguen de las facturas. Los restaurantes nuevos lo reciben listo al crearse.",
      de: "Das Inventar führt den Bestand fast von allein: du fotografierst die Lieferantenrechnung und der Wareneingang wird automatisch erfasst (neue Zutaten werden mit angelegt); Mindestbestände berechnen sich selbst, und bei Unterbestand erzeugst du die Lieferantenbestellung zum Versand per WhatsApp. Das Schwund-Panel zeigt, wo Ware verloren geht. Zutaten sind nach Lagerkategorie abgelegt (Fleisch, Fisch, Gemüse, Milchprodukte, Wein, Verbrauchsmaterial…): die Kategorie wird automatisch aus dem Produktnamen vergeben und lässt sich in der Zeile korrigieren. Die Kategorie-Buttons oben filtern die Liste, die 20 Produkte pro Seite zeigt. Jedes Produkt hat seine Einheit aus einem vollständigen Katalog (mg, g, hg, kg, ml, cl, l, Esslöffel, Stück, Packungen, Kartons, Flaschen…): änderst du die Einheit, werden Bestand, Mindestbestand und Kosten automatisch umgerechnet. Ist das Lager leer, füllt der Button „Standardlager laden“ es mit den Grundzutaten einer Profiküche – bereits in der CRM-Sprache übersetzt und kategorisiert; die Kosten bleiben bei null, bis du sie einträgst oder sie aus Rechnungen kommen. Neue Restaurants erhalten es bei der Erstellung fertig.",
    },
    links: [{ href: "/inventory", label: { it: "Apri Inventario", en: "Open Inventory", es: "Abrir Inventario", de: "Inventar öffnen" } }],
  ...
  },
  {
    ...
    keywords: ["food cost", "foodcost", "costo piatto", "ricetta", "ricette", "margine", "margini", "recipe", "recipes", "margin", "dish cost", "coste plato", "receta", "recetas", "margen", "rezept", "wareneinsatz", "prezzo suggerito", "quanto guadagno", "profitto", "profit", "beneficio", "gewinn", "piatti più redditizi", "best seller", "più venduti", "cavallo di battaglia", "menu star", "soldi lasciati sul tavolo", "money on the table", "ordina piatti", "sort dishes", "quali piatti rendono di più", "genera ricetta", "genera ricette", "ricetta automatica", "ricette automatiche", "ingredienti automatici", "auto recipe", "generate recipe", "generar receta", "rezept generieren", "aggiungi ingrediente", "ingrediente a mano", "quantità ricetta", "scarto", "unità ricetta", "add ingredient", "manual recipe", "waste percentage", "añadir ingrediente", "merma", "zutat hinzufügen", "schwund"],
    answer: {
      it: "Nel Food Cost colleghi a ogni piatto la sua ricetta (ingredienti e quantità): il CRM calcola costo, margine e incidenza, e ti suggerisce il prezzo giusto. Non devi scriverle a mano: apri un piatto e premi «Suggerisci ricetta», oppure in alto «Genera ricette mancanti» per tutti i piatti senza ricetta in un colpo — l'IA propone gli ingredienti (è una stima da verificare) e li aggancia a quelli del tuo Inventario. In alto vedi il profitto stimato a 30 giorni (margine × venduti) e il food cost medio pesato sulle vendite reali. I piatti sono raggruppati per categoria e puoi ordinarli per «Prima da sistemare», «Più redditizi», «Food cost % peggiore» o «Più venduti»; la stella ⭐ segna i cavalli di battaglia. I costi ingredienti arrivano dall'Inventario e si aggiornano da soli. Puoi anche aggiungere gli ingredienti a mano: cerchi il prodotto, scrivi la quantità nell'unità che preferisci (150 g anche se il prodotto è a chili — la conversione la fa il CRM), imposti la percentuale di scarto e vedi subito quanto costa quella riga prima di salvarla.",
      en: "In Food Cost you link each dish to its recipe (ingredients and quantities): the CRM computes cost, margin and incidence, and suggests the right price. You don't have to type them by hand: open a dish and hit 'Suggest recipe', or use 'Generate missing recipes' at the top to do every recipe-less dish at once — the AI proposes the ingredients (an estimate to review) and snaps them onto your Inventory items. At the top you see estimated 30-day profit (margin × units sold) and the average food cost weighted by real sales. Dishes are grouped by category and you can sort them by 'To do first', 'Most profitable', 'Worst food cost %' or 'Best sellers'; the ⭐ marks the menu stars. Ingredient costs come from Inventory and update automatically. You can also add ingredients by hand: search the product, type the quantity in whichever unit you like (150 g even if the product is stocked in kilos — the CRM converts it), set the waste percentage, and see what that line costs before you save it.",
      es: "En Food Cost vinculas cada plato a su receta (ingredientes y cantidades): el CRM calcula coste, margen e incidencia, y te sugiere el precio adecuado. No hace falta escribirlas a mano: abre un plato y pulsa 'Sugerir receta', o usa arriba 'Generar recetas que faltan' para todos los platos sin receta de una vez — la IA propone los ingredientes (una estimación a revisar) y los vincula a los de tu Inventario. Arriba ves el beneficio estimado a 30 días (margen × vendidos) y el food cost medio ponderado por ventas reales. Los platos se agrupan por categoría y puedes ordenarlos por 'Primero por resolver', 'Más rentables', 'Peor food cost %' o 'Más vendidos'; la ⭐ marca los platos estrella. Los costes vienen del Inventario y se actualizan solos. También puedes añadir ingredientes a mano: buscas el producto, escribes la cantidad en la unidad que prefieras (150 g aunque el producto esté en kilos — el CRM lo convierte), pones el porcentaje de merma y ves lo que cuesta esa línea antes de guardarla.",
      de: "Im Food Cost verknüpfst du jedes Gericht mit seinem Rezept (Zutaten und Mengen): das CRM berechnet Kosten, Marge und Anteil und schlägt den richtigen Preis vor. Du musst sie nicht von Hand eintippen: öffne ein Gericht und klicke 'Rezept vorschlagen', oder nutze oben 'Fehlende Rezepte erstellen' für alle Gerichte ohne Rezept auf einmal — die KI schlägt die Zutaten vor (eine zu prüfende Schätzung) und verknüpft sie mit deinem Inventar. Oben siehst du den geschätzten 30-Tage-Gewinn (Marge × verkaufte Einheiten) und den nach echten Verkäufen gewichteten Wareneinsatz. Gerichte sind nach Kategorie gruppiert und du kannst sie nach 'Zuerst erledigen', 'Rentabelste', 'Schlechtester Wareneinsatz %' oder 'Bestseller' sortieren; der ⭐ markiert die Menü-Stars. Zutatenkosten kommen aus dem Inventar und aktualisieren sich automatisch. Du kannst Zutaten auch von Hand hinzufügen: Produkt suchen, Menge in der gewünschten Einheit eingeben (150 g, auch wenn das Produkt in Kilo geführt wird — das CRM rechnet um), Schwundanteil setzen und sehen, was diese Zeile kostet, bevor du speicherst.",
    },
